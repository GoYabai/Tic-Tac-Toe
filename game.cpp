 ...
 */
const std::string VERSION = "0.4.25022870";
...
bool selectSize(int* size) {
    ...
    std::string mess = "Board will be ";
    mess += std::to_string(*size);
    mess += " x ";
    mess += std::to_string(*size);
...
bool selectGoal(int* goal, const int size) {
    ...
    std::string mess = "Goal will be ";
    mess += std::to_string(*goal);
    GameLogger::log(mess, GameLogger::Level::INFO);
    return true;
}
...
void startGame(const RunConfig& config,
    ...
    {
        clearScreen();
        GameLogger::log("Screen cleared successfully", GameLogger::Level::INFO);
    }
    // 2. Show game title
    ...
    while (true)
    {
        ...
        if (stt)
        {
            GameLogger::log("Size chosen successfully", GameLogger::Level::INFO);
            break;
        }
        else
        {
            GameLogger::log("Invalid size", GameLogger::Level::ERROR);
        }
    }
    ...
    while (true)
    {
        ...
        if (stt)
        {
            GameLogger::log("Goal chosen successfully", GameLogger::Level::INFO);
            break;
        }
        else
        {
            GameLogger::log("Invalid goal", GameLogger::Level::ERROR);
        }
    }
    // 5. Ask for game mode
    ...
    while (true)
    {
        ...
        if (stt)
        {
            GameLogger::log("Game mode chosen successfully", GameLogger::Level::INFO);
            break;
        }
        else
        {
            GameLogger::log("Invalid game mode", GameLogger::Level::ERROR);
        }
    }
    // 6. If mode == PVE
    ...
    initBoard(gameSetup.board, gameSetup.size);
    GameLogger::log("Board created successfully", GameLogger::Level::INFO);
}
...
void endGame(const RunConfig& config,
    ...
    if (config.judge_mode == true)
    {
        printResult(gameResult);
    }

    // 3. (optional) log result using GameLogger
    if (config.to_file == true)
    {
        std::string logMess;
        if (gameResult.winner == -1)
        {
            logMess = "Game ended in a draw after " + std::to_string(gameResult.turns) + " turns.";
        }
        else
        {
            ...
            int playerNum = gameResult.winner + 1;
            logMess = "Player " + std::to_string(playerNum) + " (" + playerType + ") won the game after " + std::to_string(gameResult.turns) + " turns.";
        }
        GameLogger::log(logMess, GameLogger::Level::INFO);
    }
}
